...
mipmaps are already allocated when creating the texture, so resolution will be correct in all the subpasses.
...
if a naive walk is proving not performant, we can use spatial.grid


NOTE:
point lights will have to use separate machinery from environment probes.
