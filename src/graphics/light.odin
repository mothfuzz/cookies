...
Directional_Light_Uniforms :: struct #packed {
    direction: [4]f32,
    color: [4]f32,
    view_to_shadow: [DIRECTIONAL_CASCADES]matrix[4,4]f32,
...
lights_layout_entries := []wgpu.BindGroupLayoutEntry{
    ...
    //point light depth shadows
    wgpu.BindGroupLayoutEntry{
        binding = 6,
        visibility = {.Fragment},
        texture = {sampleType = .Depth, viewDimension = .CubeArray, multisampled=false},
    },
    //point light color shadows
    wgpu.BindGroupLayoutEntry{
        binding = 7,
        visibility = {.Fragment},
        texture = {sampleType = .Float, viewDimension = .CubeArray, multisampled=false},
    },
    //directional light depth shadows
    wgpu.BindGroupLayoutEntry{
        binding = 8,
        visibility = {.Fragment},
        texture = {sampleType = .Depth, viewDimension = ._2DArray, multisampled=false},
    },
    //directional light color shadows
    wgpu.BindGroupLayoutEntry{
        binding = 9,
        visibility = {.Fragment},
...
make_point_light :: proc(position: [3]f32, color: [4]f32 = 1, range: f32 = 0, render_shadows: bool = true) -> (pl: Point_Light) {
    ...
    pl.color = color
    if range == 0 {
        pl.range = linalg.sqrt(color.a / LIGHT_MINIMUM)
    } else {
        pl.range = range
    }
...
make_spot_light :: proc(position: [3]f32, direction: [3]f32, inner_angle: f32, outer_angle: f32 = 0, color: [4]f32 = 1, range: f32 = 0, render_shadows: bool = true) -> (sl: Spot_Light) {
    ...
    sl.color = color
    if range == 0 {
        sl.range = linalg.sqrt(color.a / LIGHT_MINIMUM)
    } else {
        sl.range = range
    }
...
Lights :: struct {
    ...
    shadow_cameras: []Shadow_Camera_Draw, //point*6, directional*cams*cascades, spot
...
calculate_lights :: proc(lights: []Light_Draw, cameras: []Camera_Draw, scene_extents: [2][3]f32, rebind: bool) -> Lights {
    ...
    pl_shadow_cameras := make([dynamic]Shadow_Camera_Draw)
    defer delete(pl_shadow_cameras)
    dl_shadow_cameras := make([dynamic]Shadow_Camera_Draw)
    defer delete(dl_shadow_cameras)
    sl_shadow_cameras := make([dynamic]Shadow_Camera_Draw)
    defer delete(sl_shadow_cameras)
    ...
    for light in lights {
        ...
        case Point_Light:
            ...
            if pl.render_shadows {
                ...
                for i in 0..<6 {
                    ...
                    shadow_cam.layer = uint(pl.shadow_index*6+i)
                    shadow_cam.target = .Point
                    append(&pl_shadow_cameras, shadow_cam)
                }
            }
        ...
        case Directional_Light:
            ...
            if dl.render_shadows {
                ...
                if linalg.abs(linalg.dot(dl.direction, world_up)) >= 0.9 {
                    world_up = {0, 0, 1}
                }
                ...
                for cam, i in cameras {
                    ...
                    for c in 1..=DIRECTIONAL_CASCADES {
                        near, far := splits[c-1], splits[c]
                        shadow_cam := calculate_shadow_camera_directional(dl.direction, world_up, near, far, scale_x, scale_y, cam.inv_view, scene_aabb)
                        shadow_cam.layer_mask = light.layer_mask
                        shadow_cam.layer = uint((dl.shadow_index * len(cameras) + i) * DIRECTIONAL_CASCADES + (c - 1))
                        shadow_cam.target = .Directional
                        append(&dl_shadow_cameras, shadow_cam)
                    }
                }
            }
        ...
        case Spot_Light:
            ...
            if sl.render_shadows {
                ...
                shadow_cam := calculate_shadow_camera_positional(sl.position, sl.direction, world_up, sl.outer_angle * 2, 0.1, sl.range)
                shadow_cam.layer_mask = light.layer_mask
                shadow_cam.layer = uint(sl.shadow_index)
                shadow_cam.target = .Spot
                append(&sl_shadow_cameras, shadow_cam)
            }
    ...
    shadow_cameras := make([dynamic]Shadow_Camera_Draw, 0, len(pl_shadow_cameras) + len(dl_shadow_cameras) + len(sl_shadow_cameras))
    append(&shadow_cameras, ..pl_shadow_cameras[:])
    append(&shadow_cameras, ..dl_shadow_cameras[:])
    append(&shadow_cameras, ..sl_shadow_cameras[:])
...
shadow_offsets :: proc(lights: Lights, cameras: []Camera_Draw) -> (offsets: Shadow_Offsets) {
    ...
    offsets.num_cameras = len(cameras)
    return
}

calculate_lights_uniforms :: proc(lights: Lights, camera: Camera_Uniforms, camera_index: int, offsets: Shadow_Offsets) -> Lights_Uniforms {
    ...
    for &dl, i in directional_light_uniforms {
        ...
        if dl_in.render_shadows {
            ...
            for c in 0..<DIRECTIONAL_CASCADES {
                ...
                dl.view_to_shadow[c] = light_cam.projection * light_cam.view * camera.inv_view
                dl.cascade_splits[c] = light_cam.split_far
            }
        } else {
            dl.shadow_index = -1
        }
    ...
    for &sl, i in spot_light_uniforms {
        ...
        if sl_in.render_shadows {
            ...
            sl.view_to_shadow = light_cam.projection * light_cam.view * camera.inv_view
        } else {
            sl.shadow_index = -1
        }
    }
...
realloc_light_buffers :: proc(lights: Lights, num_cameras: int) {
    ...
    if rebind {
        ...
        bindings := []wgpu.BindGroupEntry{
            ...
            {binding = 6, textureView=ren.point_light_shadow_depth.view},
            {binding = 7, textureView=ren.point_light_shadow_color.view},
            {binding = 8, textureView=ren.directional_light_shadow_depth.view},
...
write_light_buffers :: proc(lights: Lights, cameras: []Camera_Draw, offsets: Shadow_Offsets) {
    ...
    for camera, i in cameras {
        lights_uniforms := calculate_lights_uniforms(lights, camera, i, offsets)
